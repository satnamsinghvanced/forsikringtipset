...
export async function generateMetadata() {
    ...
    return generatePageMetadata({
        title: metaTitle || "Sitemap | Forsikringtipset.no",
        description: metaDescription || "Overview of all pages on Forsikringtipset.no",
        path: "/sitemap",
        keywords: metaKeywords
            ? metaKeywords.split(',').map((k: string) => k.trim()).filter(Boolean)
            : ["sitemap", "forsikringtipset", "website overview", "site structure", "pages"],
        type: "website",
        image: metaImage || null,
        ogTitle: ogTitle || metaTitle || "Sitemap | Forsikringtipset.no",
        ogDescription: ogDescription || metaDescription || "Explore all the pages on Forsikringtipset.no",
