...
export async function generateMetadata({ params }: SlugPageProps) {
  ...
  return generatePageMetadata({
    title: metaTitle || ` | Forsikringtipset.no`,
    description: metaDescription || `Read expert articles about ${title} on Forsikringtipset.no.`,
    path: `articles/${articleCategory}/${canonicalUrl}`,
    keywords: metaKeywords ? metaKeywords.split(",").map((k: string) => k.trim()).filter(Boolean) : ["forsikringtipset", "real estate", "articles"],
    type: ogType || "website",
    image: ogImage || null,
    ogTitle: ogTitle || metaTitle || `${title} | Forsikringtipset.no`,
    ogDescription: ogDescription || metaDescription || `Explore helpful ${title} articles from Forsikringtipset.no.`,
...
const ArticleSlugPage = async ({ params }: SlugPageProps) => {
  ...
  console.log('title articleSlug:-- ', param.articleSlug)
